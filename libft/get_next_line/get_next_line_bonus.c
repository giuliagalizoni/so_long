...
/*   Updated: 2024/12/20 12:00:20 by ggalizon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd >= 1024)
		return (NULL);
	bytes_read = read_and_keep(fd, &leftovers[fd]);
	if (bytes_read < 0 || (!leftovers[fd] && bytes_read == 0))
	{
		free(leftovers[fd]);
		return (NULL);
	}
	if (leftovers[fd] && *leftovers[fd] != '\0')
	{
		line = set_line(&leftovers[fd]);
		return (line);
	}
	if (leftovers[fd])
	{
